...
const isoKeys: string[] = Object.keys(iso);

Object.keys(lcid).map((id: string) => {
  const locale = lcid[id];
  const isoLanguage = isoKeys.find(
    name => name.toLowerCase() === locale.language.toLowerCase(),
  );
...
export default function findLocale(locale: string): string {
  ...
  else if (list.find(d => d.tag === `${locale}-${locale.toUpperCase()}`))
