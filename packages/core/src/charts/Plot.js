    ...
    if (showLineLabels) {
      ...
      if (lineData.length) {
        ...
        labelWidths = lineData
          ...
          .sort((a, b) =>
            yDomain[1] > yDomain[0] ? a.value - b.value : b.value - a.value
          )
          .filter((d, i, arr) => {
    ...
    const shapeConfig = {
      ...
      x0:
        discrete === "x"
          ? d => (d.x2 ? x(d.x2, "x2") : x(d.x))
          : x(
              typeof this._baseline === "number" ? this._baseline : domains.x[0]
            ),
      ...
      y0:
        discrete === "y"
          ? d => (d.y2 ? y(d.y2, "y2") : y(d.y))
          : y(
              typeof this._baseline === "number" ? this._baseline : domains.y[1]
            ) - yOffset,
      y1:
        discrete === "y"
          ? null
    ...
    shapeData.forEach(d => {
      ...
      if (d.key === "Bar") {
        ...
        if (scaleType !== "Point" && vals.length === 2) {
          ...
          allPositions.push(
            range[1] + range[1] - allPositions[allPositions.length - 1]
          );
      ...
      if (d.key === "Line") {
        ...
        for (let e = 0; e < globalEvents.length; e++)
          markers.on(globalEvents[e], (d, i, x, event) =>
            this._on[globalEvents[e]](d.data, d.i, x, event)
          );
        for (let e = 0; e < shapeEvents.length; e++)
          markers.on(shapeEvents[e], (d, i, x, event) =>
            this._on[shapeEvents[e]](d.data, d.i, x, event)
          );
        for (let e = 0; e < classEvents.length; e++)
          markers.on(classEvents[e], (d, i, x, event) =>
            this._on[classEvents[e]](d.data, d.i, x, event)
          );
