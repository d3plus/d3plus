...
/**
    @type D3Selection
    @desc A permissive D3 selection type that accepts any generic parameterisation.
...
/**
    @type D3Transition
    @desc A D3 transition type derived from the return type of d3-transition's transition function.
*/
export type D3Transition = ReturnType<typeof transition>;

/**
    @interface Attrable
    @desc Anything that supports `.attr(name, value)` — both Selection and Transition.
*/
...
/**
    @interface Stylable
    @desc Anything that supports `.style(name, value)` — both Selection and Transition.
*/
