    ...
    selection.each(function(data, i) {
      ...
      viz_enter.append('rect')
        ...
        .on(vizwhiz.evt.click,function(d){
          clicked = null
          d3.select(this_selection).call(viz.highlight())
        })
      ...
      node.enter().append("circle")
        ...
        .on(vizwhiz.evt.over, function(d){
          if (!clicked) d3.select(this_selection).call(viz.highlight(d.id))
        })
        .on(vizwhiz.evt.out, function(d){
          if (!clicked) d3.select(this_selection).call(viz.highlight())
        })
        .on(vizwhiz.evt.click, function(d){
          if (!clicked) clicked = true
          else d3.select(this_selection).call(viz.highlight(d.id))
        })
      ...
      node.transition().duration(timing)
        ...
        .attr("fill", function(d){
          ...
          if (data[d.id].rca >= rca_cutoff) return color;
        ...
        .attr("stroke", function(d){
          if (highlight == d.id) return "#ff0000"
          var color = data[d.id].color ? data[d.id].color : vizwhiz.utils.rand_color()
          if (data[d.id].rca >= rca_cutoff) return d3.rgb(color).darker().darker().toString();
          else if (spotlight) return "#dedede";
          else return d3.rgb(color).darker().toString()
        })
        .attr("stroke-width", function(d){
          if(data[d.id].rca >= rca_cutoff) return 2;
          else return 1;
        })
        
      link
        .attr("stroke", function(d) {
          if (d.source.id === highlight || d.target.id === highlight) return "#ff0000"
          return "#dedede"
        })
        .attr("stroke-width", "1px");
